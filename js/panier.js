const panierArticles = document.querySelector('.panier-articles');
 ...
 function mettreAJourPanier(panier) { // Ajoutez le paramètre 'panier'
 ...
 function mettreAJourEvenementsQuantite() {
     ...
     quantiteInputs.forEach(input => {
         input.addEventListener('change', () => {
             changerQuantite(input.dataset.nom, parseInt(input.value) - (JSON.parse(localStorage.getItem('panier')) || []).find(item => item.nom === input.dataset.nom).quantite);
         });
     });
 }
 ...
 // chager le panier au chargement de la page
 chargerPanier();
